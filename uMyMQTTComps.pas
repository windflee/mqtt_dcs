...
uses
  ...
  Net.CrossSslSocket.Base, Net.CrossSslSocket, Net.OpenSSL, Net.SocketAPI,
  Net.CrossSocket.Base, Net.CrossSocket,
  uMyMQTT, uSimpleThreadTimer;
...
begin
  ...
  begin
    ...
    begin
      ...
      FSocket := TCrossSslSocket.Create(0);
    end
    else
      FSocket := TCrossSocket.Create(0);
  ...
  FSocket.Connect(Self.Host, Self.FPort);
end;
...
var LConns: TArray<ICrossConnection>;
begin
  if FIsConnected then
  begin
    ...
    try
      if (LConns <> nil) then
      begin
        if LConns[0].ConnectStatus = csConnected then
        begin
          LConns[0].SendBytes(data,
...
var LConns: TArray<ICrossConnection>;
    bs: TBytes;
begin
  setLength(bs, stream.Size);
  stream.Position := 0;
  stream.Read(bs, stream.Size);
  SendData(bs);
//  if FIsConnected then
//  begin
//    LConns := FSocket.LockConnections.Values.ToArray;
...
procedure TMyMQTTClient.AfterConnected;
var
  aClientID: UTF8String;
...
begin
  ...
  begin
    ...
    aClientID := ClientID;
    if aClientID = '' then
      aClientID := 'DCSMQTT_' + TimeString;
    if Assigned(FOnClientID) then
      FOnClientID(Parser, aClientID);
    ClientID := aClientID;
    if Parser.Clean then
    begin
      InFlight.Clear;
...
procedure TMyMQTTClient.BeforeDisconnected;
begin
  Mon ('Link Closed...');
  if FPingTimer <> nil then
  begin
...
procedure TMyMQTTClient.OnConnected(const Sender: TObject;
...
begin
  mon('recv data size ' + inttostr(data.Size));
  Parser.Parse (data);
end;

procedure TMyMQTTClient.OnDisConnected(const sender: TObject;
  const AConnection: ICrossConnection);
begin
  mon('DisConnected...');
  FIsConnected := false;
  if Assigned(FOnOffline) then FOnOffline(Self, false);
end;

procedure TMyMQTTClient.OnReceived(const sender: TObject;
...
procedure TMyMQTTClient.OnSent(const sender: TObject;
  const AConnection: ICrossConnection; const ABuf: Pointer;
  const ALen: Integer);
begin
  mon('send data size: ' + ALen.ToString)
end;
